...
**Manifest**:
The canonical on-disk schema soma consumes, identical across all task types: a `dataset.csv` (`sample_id`, `image_path`, and exactly one supervision column — `label` / `label_mask_path` / `points_path`, chosen by `dataset_type` — plus optional `mask_path` (precomputed tissue mask), `patient_id`, `group_id`, `spacing_at_level_0`), a `splits.csv` (`sample_id`, `split`, `fold`), and a `summary.json`. `group_id` names a non-independent sample group for representation evaluation and remains optional for ordinary task runs. `spacing_at_level_0` is a finite positive µm/px declaration for the source image's level-0 pixels, not a requested extraction spacing. The Manifest is validated against `dataset_type` at load and written by one shared `write_manifest`.
