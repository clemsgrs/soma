"""Build ``walkthrough-slide-mil.ipynb``.

Authoring tooling (not part of the tutorial). Assembles the slide-level MIL
walkthrough — a tile encoder embedded per tile, pooled by a MIL aggregator — and
writes it *unexecuted* (committed notebooks ship without outputs). See
``_build_tile_level.py`` for the build-only pattern.
"""
...
OUT = HERE / "walkthrough-slide-mil.ipynb"
