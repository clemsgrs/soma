...
def test_tile_index_is_aligned_and_contiguous():
    contours = _make_contour_result()
    result = generate_tiles(
        slide_dimensions=(1000, 1000),
        contours=contours,
...
def test_strict_threshold_fewer_tiles():
    ...
    contours = detect_contours(mask, slide_dimensions=(1000, 1000), a_t=0)
...
def test_coordinates_are_in_canonical_x_then_y_order():
    contours = _make_full_tissue_contour(slide_w=2000, slide_h=2000)
    result = generate_tiles(
        slide_dimensions=(2000, 2000),
        contours=contours,
        requested_tile_size_px=256,
    ...
    if len(result.coordinates) > 1:
        order = np.lexsort((result.coordinates[:, 1], result.coordinates[:, 0]))
        np.testing.assert_array_equal(order, np.arange(len(result.coordinates)))
...
def test_num_workers_produces_same_result():
    ...
    np.testing.assert_array_equal(result_1.tile_index, result_2.tile_index)
    np.testing.assert_array_equal(result_1.coordinates, result_2.coordinates)
    np.testing.assert_array_equal(result_1.tissue_fractions, result_2.tissue_fractions)
