...
ships fixed 1024² cell patches so it needs no tiling; MIDOG 2022 and MONKEY ship **large,
variable-size ROIs** (thousands of px, many distinct sizes), so we cut each ROI into fixed
``tile_size²`` tiles here. The tiled manifest is then extracted/trained exactly like
OCELOT, and the score path stitches per-tile predictions back to the ROI frame for the
native per-ROI metric.
...
pokes in is an unlabeled partial object) is bounded by ``overlap >= object diameter`` and
is an accepted residual (issue #287, evaluated and closed as not-planned).
...
**Spacing (flat images).** These tiles are flat PNG crops whose ``spacing_at_level_0``
declares the physical scale of their native pixels. Extraction is spacing-aware: when the
run's ``requested_spacing_um`` matches the declaration within tolerance the decoded pixels
are preserved exactly, and the persisted provenance records source == effective spacing so
the point transform is the identity. A run still declares **one** ``target_size`` at
**one** spacing, so this tiler requires the source manifest's ``spacing_at_level_0`` to be
**uniform** across ROIs (a heterogeneous flat-image manifest can't share one geometry —
resample the ROI pixels to a common spacing before tiling, never merely restamp the
declaration) and carries that single value onto every tile; ``target_spacing`` optionally
asserts it equals an expected value (``0.25`` for MIDOG).
...
def _resolve_uniform_spacing(roi_df: pd.DataFrame, target_spacing: float | None) -> float | None:
    """The single ``spacing_at_level_0`` all ROIs share, or ``None`` when the column is absent.

    A dense extraction run declares one ``target_size`` at one spacing, so tiling is only
    well-defined if every ROI shares one spacing. A heterogeneous flat-image manifest
    (e.g. re-stamped with true per-scanner mpp) can't share one geometry, so it is rejected
    here; ``target_spacing`` additionally pins the expected value.
    """
    if "spacing_at_level_0" not in roi_df.columns:
    ...
    if len(values) != 1:
        raise ValueError(
            ...
            f"at one spacing), but the manifest carries {values}. Resample the ROI pixels "
            f"to a common spacing before tiling — do not merely restamp the declaration."
        )
    spacing = values[0]
    if target_spacing is not None and round(float(target_spacing), 6) != spacing:
        raise ValueError(
            f"manifest spacing_at_level_0 {spacing} != --target-spacing {target_spacing}; "
            f"the tiles would be declared at a different spacing than the run expects."
        )
    return spacing
...
def tile_detection_manifest(
    ...
    *,
    ...
    summary = {
        ...
        "tile_size": tile_size,
        "overlap": overlap,
        "spacing_at_level_0": uniform_spacing,
...
def main(argv: list[str] | None = None) -> None:
    ...
    ap.add_argument("--target-spacing", type=float, default=None,
                    help="assert the manifest's uniform spacing_at_level_0 equals this (e.g. 0.25)")
